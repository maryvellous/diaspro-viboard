  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 p-6 bg-black/70 backdrop-blur-md animate-fadeIn select-none">
      <div className="w-full max-w-2xl dashboard-card bg-[#1e1333] text-white flex flex-col overflow-hidden shadow-2xl border-white/30">
        {/* Search Input Bar */}
        <div className="p-5 border-b border-white/15 flex items-center gap-4 bg-[#1e1333]">
          <Search className="w-5 h-5 text-[#9D85C6] shrink-0" />
          <input
            type="text"
            autoFocus
            ...
            className="flex-1 bg-transparent border-none outline-none text-white text-base font-semibold placeholder-[#9D85C6]/50"
          />
          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-white/10 text-[#9D85C6] hover:text-white"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="max-h-96 overflow-y-auto p-4 flex flex-col gap-3">
          {projects.length === 0 ? (
            <p className="text-xs text-[#A5C4DC] text-center py-8 font-mono">
            ...
            projects.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="p-4 rounded-2xl bg-[#2b1c47] hover:bg-white/15 border border-white/10 flex items-center justify-between group transition-all"
                >
                  <div
                    onClick={() => {
                      setActiveProject(p);
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <FolderGit2 className="w-4 h-4 text-[#9D85C6] shrink-0" />
                      <span className="text-sm font-black text-white group-hover:text-[#E8D19E]">
                        {displayName}
                      </span>
                      <span className="badge-pill bg-[#1e1333] text-[#A5C4DC] border border-[#A5C4DC]/30">
                        {p.branch}
                      </span>
                    </div>
                    <p className="text-xs text-[#A5C4DC] font-mono truncate mt-1">
                      {p.path}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="p-2 rounded-xl bg-white/10 hover:bg-white/20 text-[#9D85C6] hover:text-white"
                    >
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-white/10 hover:bg-white/20 text-[#98A78A] hover:text-white"
                    >
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-white/10 hover:bg-white/20 text-[#A5C4DC] hover:text-white"
                    >
