      ...
      const server = http.createServer(async (req, res) => {
        try {
          ...
          if (errorParam) {
            ...
            server.close();
            return resolve({ success: false, error: `Google OAuth error: ${errorParam}` });
          }
          ...
          if (code) {
            ...
            res.end(`
              ...
              <body style="font-family: sans-serif; background: #1e1333; color: white; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0;">
                <div style="text-align: center; background: #2b1c47; padding: 40px; border-radius: 20px; border: 1px solid #9D85C6;">
                  <h1 style="color: #9D85C6; margin-bottom: 10px;">Autenticazione Completata!</h1>
                  <p>L'account Google Workspace è stato collegato con successo a epicSnail.</p>
                  <p style="color: #aaaaaa; font-size: 13px;">Puoi chiudere questa scheda e tornare all'applicazione.</p>
                </div>
              </body>
              </html>
            `);
